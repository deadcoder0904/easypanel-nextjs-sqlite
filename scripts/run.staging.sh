...
DB_PATH=/data/users.staging.sqlite
...
FIRST_TIME_MIGRATION_STAGING="FIRST_TIME_MIGRATION_STAGING"
if [[ ! -e /data/$FIRST_TIME_MIGRATION_STAGING ]] || [[ $MIGRATE_DATABASE = true ]]; then
	# Place your script that you only want to run on first startup.
  echo 'Initialize database first time only'
	touch /data/$FIRST_TIME_MIGRATION_STAGING
	...
	echo "Migrating '/data/users.staging.sqlite'"
